...
wfm wfm::PwrXcor(wfm refWfm,int numSampsNeg, int numSampsPos)
{
    ...
    for(int offset = -numSampsNeg; offset < numSampsPos;offset++)
    {
        for(int i = 0; i < samples.size();i++)
        {
            ...
            result.samples[offset+numSampsNeg] =
            result.samples[offset+numSampsNeg] +
            complex<float>(sampPower(index)*refWfm.sampPower(i),0);
        }
    }
    return result;
}
...
double wfm::findMaxPowerIdx(int lowestIdx, int highestIdx)
{
    ...
    */
    ...
    double A = (rightIdx * (peakVal - leftVal) + peakIdx * (leftVal - rightVal) + leftIdx *
    (rightVal - peakVal)) / denom;
    double B = (rightIdx*rightIdx * (leftVal - peakVal) + peakIdx*peakIdx * (rightVal -
    leftVal) + leftIdx*leftIdx * (peakVal - rightVal)) / denom;
    double C = (peakIdx * rightIdx * (peakIdx - rightIdx) * leftVal + rightIdx * leftIdx *
    (rightIdx - leftIdx) * peakVal + leftIdx * peakIdx * (leftIdx - peakIdx) * rightVal) / denom;
    double result = -B / (2*A);
    return result;
}
...
void wfm::generatePulseTrain(int pulseLengthSamps, int periodLengthSamps,int numPeriods)
{
    ...
    {
        if((i%periodLengthSamps)<pulseLengthSamps) samples[i]=
