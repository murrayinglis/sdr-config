...
def update_plot_data(data):
        ...
        line_00.set_xdata(fft_freq_axis)  # Set x-axis data (assume index is x)
        line_00.set_ydata(fft_data)  # Update y-axis with the latest data
        axs[0].relim()  # Recompute limits
        axs[0].autoscale_view(True, True, True)  # Rescale the plot based on new data

        # PLOT TIME
        line_10.set_xdata(np.arange(len(data)))  # Set x-axis data (assume index is x)
        line_10.set_ydata(data)  # Update y-axis with the latest data
        ...
        line_20.set_xdata(np.arange(len(xcorr_data)))  # Set x-axis data (assume index is x)
        line_20.set_ydata(np.abs(xcorr_data))  # Update y-axis with the latest data
        ...
        # Extract pulses
        xcorr_matrix = []
        ranges = np.arange(len(template_signal)) * c / (2*fs)
        ranges = ranges[0:500]
        while (len(xcorr_data)>0):
            ...
            xcorr_matrix.append(xcorr_data_plot[0:500])
            append_signal_to_bin(file_path_res, xcorr_data_plot[0:500])
...
line_00, = axs[0].plot([], [], 'b-')  # Empty plot
line_10, = axs[1].plot([], [], 'b-')  # Empty plot
line_20, = axs[2].plot([], [], 'b-') 
line_30, = axs[3].plot([], [], 'b-') 

#line_01, = axs[0,1].plot([], [], 'b-') 
init_im = np.zeros(())
im_01 = axs[4].imshow(np.zeros((1, 1)), cmap='jet', vmin=0, vmax=1)
